...
# Version 3: Added EnrichR
# Version 4: Ready to Deploy
...
df_query = st.sidebar.file_uploader(
    'Upload your .csv/.xlsx file here with the first column as the index.',
...
if len(df_query) != 0:
    ...
    testdata = st.experimental_memo(pd.read_csv)("/Users/clara/Desktop/Ad5_seroneg_clara.csv", index_col=0)
...
def degs(df_dict, list_of_days, colorlist):
    ...
    with stack_expand:
        pval_cutoff = st.slider(label=f"Adjust p-value cutoff here", value=0.05, min_value=0.0, max_value=1.0, step=0.01)
        fc_cutoff = st.slider(label="Adjust fold-change cutoff here ", min_value= 0.0, max_value= 5.0, value=2.0, step=0.1)
    ...
    if len(df_dict) == 1:
        ...
        for df, name in zip(newdf_list, df_names):
            for tp, clrs in zip(list_of_days, colorlist):
                ...
                fc_filter_pertp = (fc_col_pertp[((fc_col_pertp >= log2fc_cutoff) | (fc_col_pertp < -(log2fc_cutoff)))]).to_frame()
            ...
            stacked1.add_trace(go.Bar(x=list_of_days, y=proportions['downcount'], name="Downregulated", marker_color="#636EFA"))
            stacked1.add_trace(go.Bar(x=list_of_days, y=proportions['upcount'], name="Upregulated", marker_color="#EF553B"))
...
def clustergram(dfx):
    ...
    with clust_expand:
        ...
        gene_list = st.text_area(label="Input list of genes here", help="Please use one of the following delimiters: line breaks, commas, or semicolons")
    ...
    if len(gene_list) == 1:
        clust_expand.warning("Please enter more than one gene in the list and/or select more than one column to plot the clustergram.")
...
def select_enrichr_dataset():
    ...
    geneset = enrichr_exp.radio(label='# Select a geneset :',
                               help='Choose BTM or Reactome geneset',
                               options=geneset_list)
    return geneset
...
def execute_enrichr(genelist, select_dataset, use_degs=False):
    ...
    if not use_degs:
        ...
        enrichr_results_exp.download_button(label="Download Enrichr dataframe", data=to_excel(enrichr_download), file_name="enrichr_analysis.xlsx")
        ...
        fig.update_layout(title="Enriched Pathways", title_x=0.5, showlegend=False)
        st.plotly_chart(fig, use_container_width=True)


##################################### Choose app to build dashboard ##################################################
choose_app = st.sidebar.multiselect("Choose an app to render in the main page 👉",
                                    options=["documentation", "volcano plot", "DEGs", "clustergram",
                                             "correlation matrix", "enrichr"])
for c in choose_app:
    with st.spinner("🔨Building your dashboard 🔨"):
        ...
        if c == "documentation":
            ...
            postdeg = st.sidebar.expander("Expand for autogenerated DEG clustergram", expanded=False)
            with postdeg:
